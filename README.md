...
* when a router receives a packet:
    + if the table tells that the destination is on an attached network: the packet goes directly to the host
    + if the table tells that the destination isn't on an attached network: the packet goes to another router
    + if the table doesn't know the destination and there is a default route: the packet goes to the default route
    + if the table doesn't know the destination  and there isn’t a default route: the packet is dropped (and the source IP is onformed)
...
# Level 1 solution
`01101000.01100011.00010111.00000001` (`104.99.23.1`) A1 min host address   
`01101000.01100011.00010111.11111110` (`104.99.23.254`) A1 max host address  
...
`01101000.01100011.00010111.00001100` (`104.99.23.12`) is already used    
  
`11010011.10111111.00000000.00000001` (`211.191.0.1`) D1 min  
`11010011.10111111.11111111.11111110` (`211.191.255.254`) D2 max
...
# Level 3 solution
`11111111.11111111.11111111.1+0000000` (`255.255.255.128`, `/27`) mask
`01101000.11000110.01000010.1+0000001` (`104.198.132.128`) min $\textsf{\color{red}(?)}$  
`01101000.11000110.01000010.1+1111110` (`104.198.132.255`) max  
...
`11111111.11111111.11111111.11+000000` (`255.255.255.192`, `/26`) we are free to choose a mask  
`01000101.00100000.01110110.10+000001` (`69.32.118.128`) B1, R min, because B1, R1, A1 are in the same network   
`01000101.00100000.01110110.10+111110` (`69.32.118.191`) B1, R max
...
`11111111.11111111.11+000000.00000000` (`255.255.192.0`) mask  
`10101010.11110010.00+010101.11111101` (`170.242.21.253`) min  
`10101010.11110010.00+010101.11111110` (`170.242.21.254`) max  

`11111111.11111111.11111111.1+0000000` (`255.255.255.128`) mask  
`00010001.00100001.01111110.0+1111101` (`17.33.126.125`) min  
`00010001.00100001.01111110.0+1111110` (`17.33.126.126`) max  
...
`01011001.01011100.11110001.1+0000001` (`89.92.241.129`) min  
`01011001.01011100.11110001.1+1111110` (`89.92.241.254`) max  
`01011001.01011100.11110001.1+1100011` (`89.92.241.227`) A1  
`01011001.01011100.11110001.1+1100100` (`89.92.241.228`) R1  

Network R2 + Internet:  
`11111111.11111111.11111111.1111+0000` (`255.255.255.240`, `/28`) mask  
...
`01100000.11000110.00001110.000000+10` (`96.198.14.2`) max  

Network R1 + R2:    
`11111111.11111111.11111111.111111+00` (`255.255.255.252`, `/30`) mask  
`01100000.11000110.00001110.111111+01` (`96.198.14.253`) min  
`01100000.11000110.00001110.111111+10` (`96.198.14.254`) max    
...
Network C + D + R2:    
`11111111.11111111.11111111.1111+0000` (`255.255.255.240`, `/28`) mask  
`10001101.11101111.11100101.1111+0001` (`141.239.229.241`) min  
`10001101.11101111.11100101.1111+1000` (`141.239.229.254`) max    
`10001101.11101111.11100101.1111+0001` (`141.239.229.241`) R23  
`10001101.11101111.11100101.1111+0010` (`141.239.229.242`) R22  
`10001101.11101111.11100101.1111+0011` (`141.239.229.243`) D1  
`10001101.11101111.11100101.1111+0100` (`141.239.229.244`) C1  
  
Network R1 + R2:    
`11111111.11111111.11111111.11+000000` (`255.255.255.192`, `/26`) mask 
`10001101.11101111.11100101.00+000001` (`141.239.229.1`) min  
`10001101.11101111.11100101.00+111110` (`141.239.229.62`) max   
`10001101.11101111.11100101.00+111101` (`141.239.229.61`) R21
`10001101.11101111.11100101.00+111110` (`141.239.229.62`) R13

Network R2 + Internet:    
`11111111.11111111.11111111.1111+0000` (`255.255.255.240`, `/28`) mask  
`10100011.10101010.11111010.0000+0001` (`163.170.250.1`) min  
`10100011.10101010.11111010.0000+1110` (`163.170.250.14`) max    
`10100011.10101010.11111010.0000+1100` (`163.170.250.12`) R12   
